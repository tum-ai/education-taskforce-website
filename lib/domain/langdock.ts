export const DEFAULT_LANGDOCK_LOGIN_URL = "https://app.langdock.com/";

export type LangdockCredentialInput = {
  label: string;
  email: string;
  password: string;
  group?: string;
...
export type LangdockImportError = {
  row: number;
  field: string;
  message: string;
};

export type LangdockParseResult = {
  credentials: LangdockCredentialInput[];
  errors: LangdockImportError[];
};

export type LangdockCredential = {
  id: string;
  label: string;
  email: string;
  password: string;
...
export type LangdockCredentialCard = LangdockCredential & {
  qrPayload: string;
  qrDataUrl: string;
};
...
export function parseLangdockCredentialsCsv(
  csv: string,
  options: { defaultLoginUrl?: string } = {},
): LangdockParseResult {
  const errors: LangdockImportError[] = [];
  const parsedRows = parseCsvRows(csv);
  const defaultLoginUrl = normalizeLangdockLoginUrl(options.defaultLoginUrl ?? DEFAULT_LANGDOCK_LOGIN_URL);

  if (!defaultLoginUrl) {
    errors.push({ row: 1, field: "loginUrl", message: "Default Langdock URL must use http or https." });
  }
  ...
  const credentials: LangdockCredentialInput[] = [];
  ...
  parsedRows.slice(1).forEach((fields, index) => {
    ...
    const loginUrl = raw.loginUrl ? normalizeLangdockLoginUrl(raw.loginUrl) : defaultLoginUrl;
    ...
    if (!loginUrl) {
      errors.push({ row: rowNumber, field: "loginUrl", message: "Langdock URL must use http or https." });
    }
    ...
    credentials.push({
      ...
      loginUrl: loginUrl || DEFAULT_LANGDOCK_LOGIN_URL,
    });
  });
...
export function normalizeLangdockLoginUrl(value: string): string | null {
  try {
